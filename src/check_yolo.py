...
from src.make_yolo import IMG_H, IMG_W, OUT
...
def read_label(txt_path):
    ...
    return [yolo_to_pixel(line) for line in lines]
...
def show(split="train", n=6):
    ...
    입력: split - "train" 또는 "val"
          n     - 그릴 장수 (6 이하)
    반환: 없음 (그림 창)
    """
    # 라벨 파일 정렬 → 시드 고정 → n개 뽑기
    txts = sorted(OUT.glob(f"labels/{split}/*.txt"))
    random.seed(42)
    picked = random.sample(txts, n)

    for i, txt in enumerate(picked):
        # 2행 3열 중 i+1번째 칸 (칸 번호는 1부터)
        ax = plt.subplot(2, 3, i + 1)
        # 같은 이름의 png 읽어서 띄우기
        ax.imshow(plt.imread(OUT / "images" / split / (txt.stem + ".png")))
