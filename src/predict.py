...
def predict_all(weights, image_dir, device): 
    ...
    rows = [] 
    for path in paths: 
        image_id = int(path.stem)  # "123.png" -> 123 
        result = model.predict(path, imgsz=IMGSZ, conf=CONF, device=device)[0]  # 1장만 넣으므로 [0] 
        rows += to_rows(result, int(image_id), model.names) 
