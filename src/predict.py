...
# ===== 설정값 =====
WEIGHTS = RUNS / NAME / "weights" / "best.pt"   # 제출할 모델 (train.py 의 NAME 실험 결과)
...
def to_rows(result, image_id, names):
    ...
    for (x1, y1, x2, y2), cls, score in zip(boxes.xyxy.tolist(), boxes.cls.tolist(), boxes.conf.tolist()):
        rows.append(
            {
                "image_id": image_id,
...
def predict_all(weights, image_dir, device):
    ...
    model = YOLO(weights)
    paths = sorted(image_dir.glob("*.png"), key=lambda p: int(p.stem))  # 숫자 순서로 정렬
    ...
    for path in paths:
        ...
        df.insert(0, "annotation_id", range(1, len(df) + 1))  # 맨 앞에 annotation_id 열 추가
        return df
