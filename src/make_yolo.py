...
from src.annotations import RAW, load_annotations

#설정값
OUT = Path(__file__).resolve().parent.parent / "data" / "yolo"
VAL_RATIO = 0.2  # 검증용 데이터 비율
SEED = 42
IMG_W,IMG_H = 976,1280  # YOLOv8 입력 이미지 크기
...
def remove_bad_images(ann):
    ...
    for name, boxes in ann.items():          # name: 이미지 파일명, boxes: 박스 리스트
        flag = True  #이미지를 정상이라 가정하고 시작                        
        for b in boxes: #박스리스트를 하나씩 확인!                     
            if not is_valid(b): #이 박스가 이상한가?
                flag = False #하나라도 이상하면  flag False               
                break #탈출                        
        if flag: # 이 이미지는 정상인가?                          
            clean[name] = boxes #clean딕셔너리에 이름과 박스리스트넣는다!
        else:
            print(f'[비정상 이미지] 제외 :{name}')#하나라도 비정상으로걸린게있음 출력하고 clean딕셔너리에 안넣는다                      
    return clean
...
def make_class_map(ann):
    ...
    class_to_idx = {class_id: i for i, class_id in enumerate(class_ids)}
...
def to_yolo_line(b, idx):
    ...
    동작:
        ...
        3. 공백으로 이어 붙인다.
    """
    # 중심 x: 왼쪽 끝 + 너비 절반, 이미지 너비로 나눔
    cx = (b["x"] + b["w"] / 2) / IMG_W 
    # 중심 y: 위쪽 끝 + 높이 절반, 이미지 높이로 나눔  
    cy = (b["y"] + b["h"] / 2) / IMG_H
    # 너비 비율 
    w = b["w"] / IMG_W
     # 높이 비율   
...
def split_by_combo(names):
    ...
    # 3. 앞에서 20%를 val 조합으로
    n_val = round(len(combos) * VAL_RATIO)
